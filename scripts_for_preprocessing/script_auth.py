...
# input_folder_path = './authors'  # The folder containing subfolders with gz files
# output_folder_path = input_folder_path
# inter_file_path = 'tempaa.json'
...
# #         for key in ['ids', 'works_count', 'cited_by_count', 'updated_date', 'created_date']:
# #             entry.pop(key, None)
...
# def remove_fields(inter_file, output_file):
#     """Processes JSON line-by-line to prevent memory overload."""
...
#                 # Remove specific fields from lists
#                 for key in ['topic_share', 'topics', 'x_concepts']:
#                     if key in entry and isinstance(entry[key], list):
#                         for obj in entry[key]:
#                             for subkey in ['display_name', 'subfield', 'field', 'domain', 'wikidata']:
#                                 obj.pop(subkey, None)
...
#     output_counter = 277  # To name output files sequentially as sources_1, sources_2, etc.
...
#                 output_file_path = os.path.join(output_folder, f"authors_{output_counter}.json")
