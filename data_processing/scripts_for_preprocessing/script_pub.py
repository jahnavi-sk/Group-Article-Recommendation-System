...
input_folder_path = './publishers'  # The folder containing subfolders with gz files
...
def remove_fields(inter_file, output_file):
    ...
    for entry in data:
        ...
        for key in ['lineage', 'ids', 'country_codes', 'image_url', 'image_thumbnail_url',
                     'works_count', 'cited_by_count', 'updated_date', 'created_date','sources_count']:
            entry.pop(key, None)
    ...
    for root, dirs, files in os.walk(input_folder):
        for file in files:
            if file.endswith('.gz'):  # Only process .gz files
                ...
                output_file_path = os.path.join(output_folder, f"publishers_{output_counter}.json")
