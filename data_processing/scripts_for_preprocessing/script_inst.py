...
input_folder_path = './institutions'  # The folder containing subfolders with gz files
output_folder_path = input_folder_path
inter_file_path = 'tempu.json'
...
def remove_fields(inter_file, output_file):
    ...
    for entry in data:
        ...
        for key in ['type_id', 'lineage', 'ids', 'country_code', 'country_id', 'image_url',
                    'image_thumbnail_url', 'works_count', 'cited_by_count', 'updated_date', 'created_date','geo','international','is_super_system']:
            entry.pop(key, None)
    ...
    for root, dirs, files in os.walk(input_folder):
        for file in files:
            if file.endswith('.gz'):  # Only process .gz files
                ...
                output_file_path = os.path.join(output_folder, f"institutions_{output_counter}.json")
