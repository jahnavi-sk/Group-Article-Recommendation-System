
'use client';

import { Card, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import Link from 'next/link';
import { User, Users } from 'lucide-react';
import { useSearchParams } from 'next/navigation';
import { Suspense } from 'react';

function ModeSelectionContent() {
  const searchParams = useSearchParams();
  const type = searchParams.get('type') || 'work';

  return (
    <div className="flex min-h-screen flex-col items-center justify-center p-4 bg-transparent">
      <div className="mb-12 text-center animate-in fade-in-0 duration-500">
        <h1 className="text-4xl font-bold tracking-tight md:text-5xl">Choose Your Mode</h1>
        <p className="mt-4 text-lg text-muted-foreground">How are you collaborating today?</p>
      </div>
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2 max-w-3xl w-full">
        <Link href={`/topic?type=${type}&mode=individual`} className="animate-in fade-in-0 slide-in-from-bottom-10 duration-700">
          <Card className="transform transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-primary/20 bg-card/50 backdrop-blur-sm">
            <CardHeader className="flex flex-col items-center justify-center p-8 text-center">
              <User className="h-16 w-16 mb-4 text-primary" />
              <CardTitle className="text-2xl font-semibold">Individual</CardTitle>
              <CardDescription className="mt-2">Get recommendations just for you.</CardDescription>
            </CardHeader>
          </Card>
        </Link>
        <Link href={`/topic?type=${type}&mode=group`} className="animate-in fade-in-0 slide-in-from-bottom-10 duration-700 [animation-delay:200ms]">
          <Card className="transform transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-accent/20 bg-card/50 backdrop-blur-sm">
            <CardHeader className="flex flex-col items-center justify-center p-8 text-center">
              <Users className="h-16 w-16 mb-4" style={{color: 'hsl(var(--accent))'}}/>
              <CardTitle className="text-2xl font-semibold">Group</CardTitle>
              <CardDescription className="mt-2">Collaborate and find something for everyone.</CardDescription>
            </CardHeader>
          </Card>
        </Link>
      </div>
    </div>
  );
}

export default function ModeSelectionPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <ModeSelectionContent />
    </Suspense>
  );
}
